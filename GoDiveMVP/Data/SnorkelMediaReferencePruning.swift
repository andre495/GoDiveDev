...
/// Removes a **`SnorkelMediaPhoto`** reference when its underlying Photos asset has been **deleted** — so the dive no
...
enum SnorkelMediaReferencePruning {
    ...
    @MainActor
    @discardableResult
    static func pruneIfAssetMissing(_ media: SnorkelMediaPhoto, modelContext: ModelContext) -> Bool {
    ...
    static func pruneMissingLibraryAssets(
        ...
        let all = (try? modelContext.fetch(FetchDescriptor<SnorkelMediaPhoto>())) ?? []
        var pruned = 0
