...
/// Progress, still picker, Fishial recognition, and user confirmation for one snorkel media item.
struct SnorkelMediaFishialIdentifySheet: View {
    ...
    private func identifyCroppedStill() async {
        ...
        do {
            ...
            if context.isPhotoSelection, let snorkelMedia = context.snorkelMedia {
                phase = .loading(.exportingSelectedStill())
                frame = try await DiveMediaFishialFrameExport.exportCroppedPhotoFrame(
                    snorkelMedia: snorkelMedia,
