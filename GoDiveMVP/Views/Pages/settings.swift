            ...
            VStack(alignment: .leading, spacing: AppTheme.Spacing.lg) {
                ...
                Toggle(isOn: $useImperialDisplayUnits) {
                    VStack(alignment: .leading, spacing: AppTheme.Spacing.sm) {
                        ...
                        Text(
                            "When on, depths show in feet, water temperature in °F, cylinder pressure in psi, and tank volume in cubic feet when the import text includes liters. Off uses metric (meters, °C, bar). Imported values are always stored the same way; this only changes how numbers appear."
                        )
                        .font(.footnote)
                        .foregroundStyle(AppTheme.Colors.secondaryText)
                        .fixedSize(horizontal: false, vertical: true)
