...
- **`DiveTankOverviewHeroPresentation`** — **landscape** + **minimized** tank tab: full-width profile chart, **no** PSI-used summary or small cylinder, **embedded sheet hidden**; **portrait** minimized unchanged (chart + cylinder + gas text, no markers) + **`DiveTankRotatePhoneHintView`** on the minimized sheet upper trailing corner.
...
## 119 - Friends, QR invites, friend-visible dives **(pushed)**

**Summary:** Friends connect via QR / shareable link; accepted friends can read friend-visible dive projections (notes and media opt-in). Local CloudKit log remains source of truth.

- **Firestore:** `friendInvites`, `friendships`, `users/{uid}/sharedDives`; rules in **`catalog-cdn/firestore.rules`** (deployed).
- **UI:** Profile → Friends (list, invite QR/share, redeem deep link, shared logbook/detail); Settings share toggles; `godive://` URL scheme.
- **Buddy link:** `DiveBuddy.linkedFirebaseUID` on accept; friends sorted first in dive buddy picker; “tagged you” on shared projections.
- **Sync:** projection upsert on import / friends open / settings change; delete on dive delete / unfriend / stop sharing / account delete; **ModelContext.didSave** observer upserts affected dives after field / media / buddy / tag / equipment edits (debounced).
- **Launch title color:** **GoDive** wordmark on launch/loading matches dark **`GoDiveLogoPin`** (~**`#A2E5FE`**) — storyboard, **`AppLaunchOverlay`**, **`LaunchScreenTitle`**.
- **Docs:** hybrid boundaries, OWASP matrix, privacy policy, user guide **`docs/friends.md`**, app summary.
- **Tests:** **`GoDiveFriendsTests`** (invite URL, redeem validation, projection notes/media opt-in, affected-dive ID resolution).

### Summary
- **Profile hamburger menu** — trailing plain **☰** opens a **right-side overlay** (~⅔ screen width) with title-only destinations in order: **Trips**, **Certifications**, **Equipment Locker**, **Buddies**, **Friends**, **Edit Profile**, **Settings**, plus **Sign out** pinned at the bottom.
- **Profile Details sheet page** — DAN insurance + featured certification move from the pinned identity into a sheet **Details** pager page (**`ProfileDetailContentPager`**); Details uses a static (non-scrolling) layout with DAN member number (**`#`** prefix, always links to [dan.org](https://dan.org/)), certification name/number/**date attained**, and cert card front preview.
- **Profile identity alignment** — name + dive count use the same **`BlueSheetPinnedSummary`** leading-accessory layout as buddy name / “dives together”; DAN + featured cert sit below.
- **Profile blue sheet** — buddy-style **`BlueSheetDetailPage`**: tagged-media hero, avatar on the seam, sheet shows name + **DAN** + most recent certification + dive count; panel fill uses **`ProfileBubbleBackgroundLayer`** (bubbles + scrim) instead of opaque overview blue.
- **Settings** and **Friends** use the same **`AppPage`** collapsible large-title chrome as Certifications / Equipment / Buddies / Trips (scroll-under header + water bubbles).

### Files
- **`settings.swift`** — **`AppPage`** + collapsible inline title; form scrolls under header.
- **`friends_list_view.swift`** — **`AppPage`** + collapsible inline title; list / empty via **`AppScrollUnderHeader*`**.
- **`SettingsPresentation.swift`** — **`pageTitle`**.
- **`profile.swift`** — **`BlueSheetDetailPage`** (tagged-media hero, seam avatar, DAN/cert/dives); side-menu overlay.
- **`ProfileSideMenuOverlay.swift`** — trailing ~⅔-width panel; **Sign out** pinned at bottom.
- **`DiveBuddyDetailHeroHeaderView.swift`** — **`.profile`** hero style.
- **`ProfilePresentation.swift`** — side-menu title helpers + width fraction.
- **`GoDiveMVPTests`** — assert **`pageTitle`**; **`profilePresentation_sideMenuTitles_arePageTitlesOnly`**.
- **Friend-share edit upsert:** **`GoDiveFriendShareRefreshCoordinator`**, **`GoDiveFriendShareAffectedDiveIDs`**, **`ContentView`** observer, field/notes sheets **`save()`**; **`GoDiveFriendsTests`** affected-ID coverage.
- **Docs:** getting-started, home, settings, friends, trips-and-buddies, index — Profile menu / tagged media paths.
- **Bug fix — invite QR 404:** QR / share / copy used `https://godiveios.com/invite/…` (Wix landing has no `/invite` route → Safari 404). **`GoDiveFriendInviteURL.preferredInviteURL`** now uses **`godive://invite/{token}`** until Universal Links ship.
- **Bug fix — invite QR first tap:** Friends invite sheet uses **`sheet(item:)`** with a ready URL/token payload so the QR is not presented before the link exists; QR renders in **`.task(id:)`** on the share sheet.
- **Friends invite sheet:** fixed **medium** detent (full half-sheet), scrollable body + slightly smaller QR so title, code, link, and actions fit comfortably.
- **`links.godiveios.com`:** Firebase Hosting invite fallback + AASA; app **`applinks:links.godiveios.com`**, Universal Link handler, preferred HTTPS invite URL. Ops: **`cursor/friend_invite_links_domain.md`**. **Multi-site Hosting:** catalog on **`godive-1cff8`**, invites on **`godive-links`** (`public-links/`) so the friend domain does not expose catalog JSON.
- **Friends UX speed:** Friends list no longer blocks on full dive republish; redeem dismisses immediately; **`goDiveFriendGraphDidChange`** refreshes Friends + Buddy Feed; parallel profile / shared-dive fetches; lighter redeem validation.
- **Bug fix — Friends endless spinner:** loading flag used stale `friends.isEmpty` in `defer`; parallel profile fetch used `@MainActor` `TaskGroup` children (MainActor deadlock).
- **Activity Log Buddy Feed:** segmented **My Activities | Buddy Feed** under the **Activity Log** title (Explore-style glass scope toggle; hides on scroll, returns at top); Buddy Feed merges friends’ shared dive projections (newest first) with read-only detail push; empty states with **Add friends** / **View friends** → **`FriendsListView`**; pull-to-refresh + auto-refresh when selecting **Buddy Feed**, switching to **Logbook** tab on that segment, popping back to the list, re-tapping **Logbook**, or foregrounding the app on visible Buddy Feed.
- **My Activities summary:** right-aligned **`x Dives | x Bottom Time`** row under the upcoming-trip banner (when shown) and above dive rows — total in-water time prefers import bottom time when present; dive count excludes logbook **`-`** (hidden number) activities (same as Profile dive total).
- **Profile side menu:** pushing Settings / Certifications / Equipment / Buddies / Friends from the **☰** menu leaves **`showsSideMenu`** set so the panel is still open after popping back to Profile.
- **Dive overview nested navigation:** **`DiveActivityOverviewUIStateStore`** saves tab, sheet detent, selected media, and panel scroll offset when leaving **`ViewSingleActivity`** for tag / species / buddy / site pushes; restores on return (skips when a logbook media deep link is still applying).
- **Tests:** **`LogbookBuddyFeedPresentation`** merge/sort + empty-kind; segment title expectations in **`GoDiveMVPTests`**.
- **Tests:** **`LogbookMyActivitiesSummaryPresentation`** dive count + bottom-time header line.
- **Field Guide 3D heroes:** drag uses a **trackball** (free **360°** rotation in any direction, pinned in place); pinch zooms in only; after **15 s** idle orientation/zoom ease back and idle spin + float resume; glow disc fixed.
- **My Activities / Home summary bottom time:** **`LogbookMyActivitiesSummaryPresentation.formattedBottomTimeHoursRounded`** — nearest whole hour, no minutes (Home + Logbook share **`headerLine`**).
- **Launch / loading:** **GoDive** title spacing under pin logo tightened (**`AppLaunchLayout.logoToTitleSpacing`** **10** pt; **`LaunchScreen.storyboard`** + **`AppLaunchOverlay`**).
- **Trip add / edit dates:** **`TripPlannerFormContent`** uses one inline **`UICalendarView`** range picker (**`TripDateRangeCalendarView`**) instead of separate start/end **`DatePicker`** rows; summary line shows the formatted range. **`DiveTripDateRangePickerPresentation`**. Test: **`diveTripDateRangePickerPresentation_normalizedDates_singleDayWhenEndMissing`**. Docs: **`trips-and-buddies.md`**.
- **Bug fix — trip date range flash:** Calendar selection updates **`startDate`** and **`endDate`** in one **`form`** write so the footer no longer briefly shows “start must be on or before end” during a valid range pick.
- **Create tag sheet:** nested **New tag** modal uses the same blue overview-panel presentation (**large** detent, no gray frosted chrome) and glass **Cancel** / **Add** toolbar buttons as notes / buddies / tags picker sheets.
- **Launch speed:** **`restoreSession`** dismisses overlay after Keychain + **`UserProfile`** fetch only; **`AppLaunchDeferredSessionReconciliation`** runs CloudKit merge + **`AppLaunchSessionValidation`** afterward. **`AppLaunchMaintenance`** splits essential vs **2 s** deferred tier (CDN, previews, heavy backfills); duplicate launch identity reconcile removed. First Home rebuild defers **150 ms**; Logbook cache defers until **Logbook** tab selected. Tests: **`appLaunchPostOverlayPresentation_*`**, **`logbookRootAppearPresentation_*`**, **`homeOverviewRebuildPresentation_initialLaunchUsesPostOverlayDefer`**.
- **Launch overlay:** **`ContentView`** does not mount until **`isRestoringSession`** is false — avoids Home painting under the splash during session restore (**`AccountSessionMainShellPresentation.shouldMountMainAppShellUnderlay`**).
- **Field Guide browse:** hub categories, category subcategory rows, and species lists sort **A–Z** by display title / common name (**`FieldGuideCatalogIndex`**).
- **Bug fix — empty Logbook after launch:** **`restoreSession`** now runs **`reconcileCloudKitIdentityIfNeeded`** before the main shell mounts so **`@Query`** owner filters match dives after CloudKit profile merge; **`TabView`** identity resets on profile id change; Logbook rebuilds list cache when rows exist but the off-tab defer left the display empty.
- **Trip add / edit sheet:** scrollable blue-panel layout (matches notes / buddies modals) fixes calendar clipping; form is **trip name** + **dates** only (countries field removed).
- **Onboarding interests:** welcome + post–sign-up activity pickers offer **Scuba Diving** and **Snorkeling** only (**`UserOnboardingActivityKind.welcomePickerKinds`**); free diving remains on profile for legacy accounts.
- **Logbook add-activity hub:** **+** opens **`LogbookAddActivityHubView`** — three full-height tiles (**New Dive Activity** → **`ActivityUploadView`**, **New Snorkel Activity** → FIT placeholder **`SnorkelActivityUploadView`**, **Connect Device** → **`ConnectDeviceComingSoonView`**). **`LogbookAddActivityPresentation`**, routes **`diveActivityUpload`** / **`snorkelActivityUpload`** / **`connectDeviceComingSoon`**. Test: **`logbookAddActivityPresentation_hubOptionsMapToRoutes`**. Docs: **`logbook.md`**.
- **Logbook My Activities summary:** **dives | bottom time** subtitle is center-aligned above the list.
- **Logbook dive rows:** scuba tiles show **`water.waves.and.arrow.trianglehead.down`** beside an oval **#** (icon outside the chip); snorkel rows show **`figure.pool.swim`** only (no **Snorkel** oval). Successful FIT / UDDF / snorkel import navigates to the inserted activity detail (bulk UDDF → newest dive).
- **Dive site location lines:** country flag emoji prefixes Explore list place lines, site detail pinned subtitles, and dive overview **region, country** headers (**`DiveSiteCountryPresentation.prefixedWithFlagEmoji`**). Tests: **`diveSiteCountryPresentation_flagEmoji_mapsCommonCatalogCountries`**.
- **Logbook My Activities loading:** while dives exist in the store but the row cache has not painted yet, show **`ProgressView`** in the header and list instead of **0 Dives | 0 Bottom Time** (**`LogbookMyActivitiesSummaryPresentation.showsLoadingChrome`**).
- **Friend invite accepted push:** **`GoDiveFirebaseCloudMessaging`** (FCM + APNs, token at **`users/{uid}/private/fcm_*`**); **`GoDiveFriendInvitePushPresentation`** / **`GoDiveFriendInvitePushTrigger`**; **`notifyFriendInviteAccepted`** Cloud Function (**`catalog-cdn/functions`**); tap → Logbook **Friends** (**`ContentView`** + Profile). Ops: **`cursor/friend_invite_push_notifications.md`**. **`FirebaseMessaging`** SPM + **`aps-environment`**. Tests: **`GoDiveFriendsTests`** push trigger + document ID.
- **Friend ↔ buddy roster:** **`GoDiveFriendBuddyLinking`** syncs all friends on Friends list load, Buddy Feed refresh, and post-launch reconciliation; fuzzy name match links **`linkedFirebaseUID`** on an existing **`DiveBuddy`**, merges duplicate name rows (**`DiveBuddyRosterMerge`**), and **`DiveBuddyOrFriendDetailView`** routes linked buddies to **`FriendProfileView`**. **`autoLinkUnlinkedBuddies`** runs after FIT/UDDF import, on dive/media buddy tags, and when adding a roster buddy. Tests: **`GoDiveFriendsTests`** link + merge + auto-link.
- **Friend profile:** **`FriendProfileView`** (Profile-style BlueSheet, blank sheet); Firestore **`profileHeroURL`** + **`profileHeroMediaKind`**; Storage **`profileHero.jpg`** / **`.mp4`**; **`GoDiveProfileHeroFirestoreSync`** from owner Profile hero; Friends list + Buddy Feed name navigation; **`catalog-cdn/storage.rules`** hero + sharedMedia paths. Tests: hero kind + Storage URL gate.
- **UDDF / import site geography:** **`DiveImportedLocationParsing`** recognizes a trailing ISO/catalog country in comma-separated **`locationName`** (e.g. MacDive **city, state, country** in the country field) so dive sites store **country** + **region** correctly and map flags match. Tests: Greenwood Village / Colorado / US cases.
- **Invite redeem → friend profile:** **`GoDiveFriendInvitePostRedeemNavigation`** — after **Connect**, Logbook pushes **`FriendProfileView`** for the inviter.
- **Detail hero media/map:** **`PushedDetailHeroModePresentation`** — no associated media → map-only header (no toggle); media + map → toggle, default **media**. Applied to profile, buddy, friend profile, tag, trip, species (tagged dive media), dive site.
- **Unified Buddies list:** **`DiveBuddiesListView`** merges local roster + Firestore friends (**`BuddiesListPresentation`**); friend badge on avatar → **`FriendProfileView`**; non-friend roster rows show **Invite** (creates link + **`FriendInviteSMSComposePresentation`** / Messages, optional Contacts **To**); toolbar **QR** + **+**; Profile menu drops separate **Friends**; Logbook **`.friends`** route still opens the same list (**`FriendsListView`** shim). Tests: **`GoDiveFriendsTests`** merge + SMS copy.
- **Bug fix — Invite SMS flash:** per-buddy **Invite** no longer uses a SwiftUI **`.sheet`** around **`MFMessageComposeViewController`**; UIKit presents Messages directly (**`FriendInviteSMSComposePresentation`**).
- **Buddy / friend map heroes:** **`BlueSheetDetailHeroBandFill`** + full-band **`TripDetailMapView`** fit (**`context.mapFitLayout()`**) — same header map framing as trip / tag detail pages.
- **Bug fix — buddy detail freeze (map-only hero):** hero **`onChange`** no longer forces **media** while map pins are deferred empty on map-only buddies (**`PushedDetailHeroModePresentation.shouldFallBackFromMapToMedia`**).
- **Bug fix — Buddies list buddy tap freeze:** value-based **`navigationDestination(for: BuddiesListNavigationRoute)`** instead of per-row **`NavigationLink { ViewDiveBuddyDetails }`** (stops eager detail **`init`** + **`@Query`** for every roster buddy); buddy detail cache seeds on **`onAppear`** not **`init`**.
- **Bug fix — Buddies row tap no-op:** Profile owns **`navigationDestination(item:)`** for **`BuddiesListNavigationRoute`**; Logbook uses **`openBuddiesListDetailRoute`** → **`LogbookRoute.buddiesListDetail`**; shared **`BuddiesListNavigationDestinationView`** host.
- **Bug fix — Buddies detail back stack:** buddy/friend detail pushes with in-list **`NavigationLink`** (Profile path) so **Back** returns to **Buddies** instead of skipping to Profile; Logbook still uses programmatic **`LogbookRoute.buddiesListDetail`**.
- **Buddies list — connected friends:** second line **dives together**; first line **N total dives** from Firestore **`totalDiveCount`** (owner sync on deferred launch maintenance).
- **Product plan:** **`cursor/product_differentiation_plan.md`** — catalog (legal/partnership/3D), friends (parity + compare + feed), snorkel (`ActivityKind`), sequencing batches, competitor audit checklist.
- **Debug — Buddies → buddy detail freeze:** **`BuddiesListNavigationDiagnostics`** (`Logger` category **BuddiesNavigation**) logs list destination appear, **`ViewDiveBuddyDetails`** init timing, body-eval count, task phases, tag **`onChange`**, content rebuild, hero mode sync, and hero map-pin fallback decisions.
- **Buddies list Invite:** tighter glass capsule (**caption2**, less padding, no min width) so **dives together** subtitle fits beside the button.
- **Friend profile avatar:** remote **`AsyncImage`** photos use the same **`ProfileAvatarChrome`** accent ring as **`ProfileAvatarView`** / buddy detail.
- **Temp — blue sheet identity layout lab:** DEBUG **Settings → Blue sheet identity layout (temp)** — drag avatar, name block, **panel hairline divider** (`panelDividerVertical`), and pager content top (`panelContentTop`); **Copy deltas** for profile / buddy / friend spacing handoff.
- **Profile / buddy / friend identity spacing:** avatar vertical **+23 pt**, identity text **+13 pt**, pinned→panel gap **+30 pt** via **`DiveBuddyDetailPresentation`** layout adjustments (shared **`identityBlueSheetPageConfiguration`**).
- **Profile sheet content width** — removed duplicate **`.horizontal`** **`lg`** inset on Details metadata ( **`BlueSheetDetailPage`** already applies **`BlueSheetDetailPagePinnedSummaryPresentation.horizontalPadding`**); matches buddy/trip blue-sheet pager width.
- **Blue sheet panel body top spacing** — shared **`pushedDetailPinnedSummaryBottomPadding`** (**`bodyBottomPadding` + 30**) on catalog pushed pages (dive site, reference site, Field Guide species, certification, equipment) via **`pushedDetailWithStandardPanelBodySpacing`**; Home tab root unchanged; profile/buddy/friend still use identity config (same padding token).
- **Blue sheet content top rule** — **`BlueSheetDetailPanelContentTopDivider`** (1 pt **`accentDeep`**: dark blue light mode, light blue dark mode) full width between pinned summary and pager on **`BlueSheetDetailPage`**; text columns keep horizontal **`lg`** inset. Pinned-summary → divider gap includes **`panelContentTopDividerVerticalAdjustment`** (**−21 pt**).
- **Trait-aware accent palettes** — **`AdaptiveAccentColor`**; Field Guide category tiles + trip logbook/detail rails use darker contrast hues in **light** mode and pastel hues in **dark** mode (**`FieldGuideCategoryAccentPresentation`**, **`LogbookTripGroupAccentPalette`** light/dark slots). Tests: category + trip luminance ordering.
- **Media** tab: embedded overview panel uses the same opaque blue **`AppOverviewSheetPanelBackground`** as map / tank (no frosted translucent chrome); **large** tagged-species scroll feathers into that solid surface.
- **Media frosted overlays** — fullscreen grid fish/buddy tag overview (and Home/trip marine-life overlays) still use **`DiveActivityMediaFrostedOverlayBackground`**; embedded dive **Media** panel no longer does.
- **Snorkel FIT import:** **`FitActivitySessionValidation`** — scuba FIT requires **Single-Gas / Multi-Gas / CCR / Gauge**; snorkel FIT requires **Snorkel** or **Open Water** swim; wrong pipeline shows import-fail alert. **`SnorkelActivity`** + **`SnorkelProfilePoint`** (GPS + HR track); **`FitSnorkelFileDecoder`** accepts Garmin **Snorkeling** or **Open Water** swim only; **`FitActivitySessionValidation`** rejects wrong activity types early (dive vs snorkel) with import-alert copy; **`FitSnorkelFileImport`** mirrors dive duplicate / site / buddy / Photos window attach; **Logbook → New Snorkel Activity** file picker wired; **`SnorkelImportAlertPresentation`** success/fail alert with imported activity count. Fixtures: **`GoDiveMVPTests/Fixtures/snorkel_garmin.fit`**, **`open_water_garmin.fit`**.
- **Snorkel activity detail:** **`ViewSingleSnorkelActivity`** — same embedded **two-detent** overview shell as dive (**minimized** + blue-sheet **large**; map / heart rate / media icon tabs). **Map** tab: site pin + snorkel stats (duration, swim distance, max depth, avg temp). **Heart rate** tab (replaces tank): GPS **swim track** polyline in the hero (**`SnorkelSwimTrackMapView`**) + panel with avg/max HR, calories, and **`SnorkelHeartRateProfileChart`**. **Media** tab: session photo pager + **`LogbookRoute.snorkelMedia`** thumbnail deep link. **`SnorkelDerivedDataBuilder`** off main actor. **`DiveActivityOverviewTabSelection.overviewDetent(whenSelectingSnorkel:)`** opens **large** on tab change. Tests: **`snorkelDerivedDataBuilder_*`**, **`snorkelSwimTrackMapPresentation_*`**, **`diveActivityOverviewTabSelection_allTabs_useLargeDetent`**.
- **Snorkel media parity with dive:** **`ActivityOverviewGalleryMedia`** + generic **`ActivityPhotosPanelContent`**, **`ActivityMediaBackgroundView`**, carousel/thumbnail/item views; marine-life + buddy tag sheets (**`SnorkelMarineLifeTagPickerSheet`**, **`SnorkelMediaBuddyTagPickerSheet`**), Fishial identify (**`SnorkelMediaFishialIdentifySheet`**), **`PhotosPicker`** import (**`SnorkelActivityMediaBatchImport`**), featured logbook preview (**`SnorkelActivityMediaStorage.setFeaturedMedia`**), preview JPEG + PhotoKit prune/repair (**`SnorkelMediaPreviewStorage`**, **`SnorkelMediaReferencePruning`**, **`GalleryMediaPhotoKit`**). Tests: **`snorkelActivityMediaStorage_setFeaturedMedia_persistsAndClears`**.
- **Home featured media — buddy chip** — tagged avatars fan **left** from the buddies icon (staggered spring); dive-site capsule + marine-life fish chip fade out while expanded and return on second tap (**`HomeMediaCarouselPresentation.taggedBuddyHorizontalOffsetX`**). Test: **`homeMediaCarouselPresentation_taggedBuddyHorizontalOffsetX_fansLeftWhenExpanded`**.
- **Depth profile charts** — **`DiveDepthProfileOverlayChart`** Y-axis extends **20%** past max depth; lightly smoothed yellow gas line; shimmering water fill + **~7×** miniature bubbles under the depth curve (**`DiveDepthProfileChartWaterFillView`**, **`WaterBubbleAnimationIntensity.chartUnderfill`**). **Minimized** tank detent (user + friend detail): compact chart with axes sits in the hero band below the PSI summary + cylinder (restored centered layout). **Large** detent: **edge-to-edge** chrome — no axes, full screen width, plot bottom flush to sheet seam (**`DiveDepthProfileChartPresentation.ChromeStyle.edgeToEdge`**). Buddy-feed tile heroes use edge-to-edge chrome, filling the **128 pt** card header (static water fill, no bubble animation). Tests: **`diveDepthProfileChartPresentation_*`**, **`diveDepthProfileOverlayChartLayout_edgeToEdgePlotRect_usesFullBounds`**, **`diveTankOverviewHeroPresentation_minimizedProfileChartFrame_isEdgeToEdgeAboveSheetSeam`**.
- **Snorkel / dive overview identity header:** **`DiveActivityMapOverviewHeader`** matches logbook leading row — scuba **`water.waves.and.arrow.trianglehead.down`** + dive **#** chip; snorkel **`figure.pool.swim`** only. Untitled site titles **New Dive Activity** / **New Snorkel Activity** on map, heart-rate, and media panels. Tests: **`snorkelActivityOverviewPresentation_siteHeaderTitle_usesNewSnorkelActivityWhenUntitled`**.
- **Dive map hero framing:** **`mapBottomContentMargin`** uses per-device **large** sheet height (**`DiveActivityOverviewSheetLayoutContext`**) and live grabber height on the **map** tab so **`GMSMapView.padding`** / MapKit **`adjustedMapCenter`** match the real panel — pin stays centered in the visible band; Google legal logo clears the sheet. **Map zoom** interpolates with panel height (**`cameraDistanceMeters(sheetHeightFraction:)`**) instead of stepping only at resting detents. — compact **minimized** (~20%, unchanged) and **large** aligned to pushed **`BlueSheetDetailPage`** panel height via **`HomeOverviewLayout.sheetSeamYFromScreenBottom`** (**`DiveActivityOverviewSheetLayoutContext`** + dynamic **`DiveActivityOverviewPanelMetrics.largeHeightFraction`**). Half-screen **medium** detent removed; default open is **large**. **`DiveActivityOverviewEmbeddedPanel`** passes layout width + safe top for per-device large height. Tests: **`DiveActivityOverviewTwoDetentTests`**.
- **Tank tab — large detent hero:** portrait **large** shows the interactive depth + gas **`DiveDepthProfileOverlayChart`** in the hero band above the blue sheet (pinch/pan; no media markers). **Minimized** keeps the small cylinder + chart stack; **landscape** shows media on the profile chart at every detent.
- **Media** tab: full-bleed pager at **minimized**; at **large** portrait, photos/videos **aspect-fill** the hero from screen top to the sheet seam (sides + top) and **interpolate to full viewport fill** while dragging toward **minimized** (**`DiveActivityMediaHeroPresentation`** + live **`sheetHeightFraction`**). Landscape stays full-bleed.
- **Tank landscape depth chart — media markers:** tap opens **`LinkedMediaFullscreenView`** (edge-to-edge browse, center play/pause, fish/buddy chrome) with capture **date/time** (+ depth/minutes subtitle when known) in the lower-leading glass chip instead of **View on dive**. **`LinkedMediaFullscreenView.Configuration.diveDepthChart`**, **`TripDetailMediaGalleryOverlayControls`** capture-timestamp mode. Tests: **`linkedMediaFullscreenPresentation_bottomLeadingCaptureTimestampLabels_*`**.
- **Tank minimized entrance:** collapsing to **minimized** on the tank tab runs a **5 s** shared **`easeInOut`** — cylinder PSI drain, depth + gas profile **`trim`** draw, and **PSI used** integer count-up (**`.numericText`**) from **0** to total consumed; **SAC** / **RMV** appear after the tally finishes. Layout reflow stays **0.45 s**.
- **Media large detent hero:** aspect-fill band bleeds **20 pt** below the sheet seam and bottom-aligns in the band so photos reach the embedded panel’s rounded top corners (no side gap at the seam).
- **Dive / snorkel overview identity icons:** scuba + snorkel SF Symbols in **`DiveActivityMapOverviewHeader`** use **24 pt** semibold (~**2×** logbook **caption**). Token: **`DiveActivityOverviewPresentation.activityIdentitySymbolPointSize`**.
- **Dive / snorkel overview panel scroll restore:** map (and tank) sheet scroll position survives nested **`NavigationLink`** pushes (e.g. Marine Life → species) — **`OverviewPanelScrollArea`** top inset + **`DiveActivityOverviewScrollRestoration`**, store-backed **`scrollRestorationFallbackY`**, live persist on scroll. Snorkel uses **`SnorkelActivityOverviewUISnapshot`**. Tests: **`diveActivityOverviewScrollRestoration_*`**, **`snorkelActivityOverviewUIStateStore_roundTripsSnapshotByActivityID`**.
- **Dive Media minimized timestamp:** capture date chip on the hero uses shared **`MediaCaptureTimestampChromeLabel`** (Liquid Glass capsule + clock icon), portrait **minimized** detent; chip is horizontally centered above the sheet (owned + friend-shared media heroes).
- **Dive Media landscape:** full-bleed pager shows grid-style chrome — center play/pause on video, Liquid Glass capture timestamp + buddy/fish (+ featured star) via **`TripDetailMediaGalleryOverlayControls`** (**`showsLandscapeGridStyleMediaChrome`**). Featured star sits **upper trailing** (same top inset as fullscreen chrome); buddy/fish stay bottom-trailing. Tap media toggles chrome visibility.
- **Search / grid media tag overview:** fish/buddy panel height uses live layout context (**`tagOverviewPanelHeight(in:)`**) so **large** detent matches dive **Media** on the same device (not presentation-reference width only).
- **Profile sheet pager (3 pages):** swipe **Diver stats** (same **`HomeLifetimeStatsSection`** + leaderboards as Home), **Details** (DAN + featured certification), **Your tagged photos** grid (**`ProfileTaggedMediaGridSection`** → fullscreen browse + featured star on self buddy). **`ProfileDetailContentPager`**, **`ProfileDetailContentPage`**. Tests: **`profileDetailContentPagerPresentation_detailsPageCopy`**.
- **Profile hero → Firebase on featured star:** starring self tagged media for the profile header calls **`GoDiveProfileHeroFeaturedMediaSync`** (force re-upload) so **`profileHeroURL`** / **`profileHeroMediaKind`** update for friends — Profile tagged-media pager and self-buddy detail path.
- **Logbook snorkel row thumbnail:** **`LogbookRowMediaPreviewView`** uses **`SnorkelActivityMediaThumbnailView`** (square **`scaledToFill`** + clip) so snorkel tiles match scuba row previews.
- **Logbook My Activities kind filter:** filter menu beside **My Activities | Buddy Feed** when **My Activities** is selected — **All activities**, **Dives**, or **Snorkels** only (not Buddy Feed). **`LogbookMyActivitiesKindFilterPresentation`**, **`LogbookMyActivitiesKindFilterMenu`**. Tests: **`logbookMyActivitiesKindFilter_filtersMergedSeeds`**, updated loading-chrome test.
- **Snorkel swim distance units:** **`DiveQuantityFormatting.swimDistance`** — logbook row, map stats, and minimized snorkel summary show **m** (metric) or **yd** (imperial). Tests: **`snorkelActivityOverviewPresentation_mapStats_swimDistanceUsesMetersOrYards`**, **`DiveQuantityFormatting.swimDistance`**.
- **Activity map weather (WeatherKit):** **Weather** section on dive + snorkel **Map** tab at **large** sheet detent — hourly conditions near entry time via **`ActivityWeatherKitService`** + **`ActivityMapWeatherConditionsSection`** (Apple **`WeatherAttribution`**). Entitlement **`com.apple.developer.weatherkit`**; link **WeatherKit.framework**. Enable the WeatherKit capability on the App ID in Apple Developer if fetches fail. Wider query window + daily fallback; **`permissionDenied`** / **`noData`** user copy. Tests: **`activityWeatherConditionsPresentation_eligibilityAndFormatting`**. **`ActivityWeatherAttributionFooter`** loads legal mark/link from **`WeatherService.shared.attribution`**; multi-dataset WeatherKit fetch uses hourly + daily **`Forecast`** tuple.
- **Swift 6 — gallery media ordering:** **`GalleryMediaOrdering`** / **`GalleryMediaOrderFields`** centralize oldest-first sort for UI + derived snapshots; model-touching **`DiveActivityMediaPresentation`** (sort, featured, Fishial match, selection helpers) and tagged-media presenters run on **`@MainActor`**; **`DiveActivity.sortedMediaPhotos`** sorts via sendable order fields — **0** project isolation warnings on device/sim builds.
- **iCloud restore after reinstall:** **`AccountRemoteDataPopulation`** on sign-in + session restore — Firebase Auth/profile, avatar restore, friend buddies, CloudKit identity merge/retries; auto-reopens SwiftData with private CloudKit when the local log is empty but mirroring was off (**`AppModelContainer.reloadProductionAfterCloudKitReconnect`**). Launch overlay stays up during population. Settings **iCloud dive log** status row. Tests: **`appSessionBootstrapPresentation_showsLaunchOverlayWhileRestoringOrPopulating`**, reconnect factory test, merge/snorkel tests.
- **Bug fix — Sign in with Apple crash:** **`completeSignIn`** is **`async`** — Firebase + optional CloudKit store reload finish **before** **`currentProfile`** attaches and the main shell mounts; **`ProductionAppRoot`** swaps **`modelContainer`** + **`.id`** after reconnect; brand-new accounts skip sticky local-only reconnect when the log is empty.
- **Bug fix — returning user + iCloud reconnect:** reconnect parks the local-only store and clears the session profile id — session now resolves **`UserProfile`** by **Apple user id** (with up to ~60s wait for CloudKit import), avoids tearing down **`AppSessionRootView`** on container swap, and defers Keychain profile id until after attach.
- **Bug fix — sign-in crash (returning + iCloud):** reconnect runs **before** any local profile write; **`isSignedIn`** stays false while population runs; container reload no longer early-attaches session; Firebase/iCloud phases re-fetch **`UserProfile`** on the live store. Sign-in screen shows **Diagnostic reports** when local crash rows exist (no account required).
- **Bug fix — sign-in crash (SwiftData reload):** Sign in with Apple **no longer reloads** the SwiftData container in-process (that swap was crashing). Returning users with a local-only store get iCloud reconnect on the **next cold launch** + an in-app alert; session restore still reconnects at launch when already signed in.
- **Bug fix — iCloud dives after sign-in:** When a returning user signs in on a local-only store, GoDive now **automatically reconnects to private CloudKit** once Home is ready (loading overlay, up to ~90s import wait). Session restore resolves the profile by **Apple ID** after reconnect clears the session profile id.
- **Bug fix — CloudKit restore empty logbook:** Identity merge no longer deletes a zero-activity CloudKit twin in favor of a SIWA-minted empty session profile; merge waits until owned dives/snorkels exist. Sticky local-only + empty store always schedules reconnect (even without ReturningAccountHints). Post-reconnect waits for owned activities before final attach/merge.
- **Bug fix — CloudKit sync dead since snorkel schema (root cause):** device diagnostics (`cloudkit-open-diagnostics.txt`) showed every CloudKit store open failing with *“CloudKit integration requires that all relationships have an inverse: SnorkelActivity: owner”* → silent local-only fallback, so no reconnect/merge logic could ever import dives. Fixes: **`UserProfile.snorkelActivitiesStorage`** (`@Relationship(deleteRule: .cascade, inverse: \SnorkelActivity.owner)`); identity merge clears `duplicate.snorkelActivities` before delete; **`cloudKitOpenPolicyVersion` → 8** so sticky local-only devices retry CloudKit once with the fixed schema; **`AppSwiftDataDualStoreMigrator.migrate`** now copies snorkel rows (**`SnorkelActivity`**, **`SnorkelMediaPhoto`**, **`SnorkelBuddyTag`**, **`SnorkelProfilePoint`**, snorkel-side **`SightingInstance`** / **`DiveMediaBuddyTag`** links) so parked-store recreation keeps snorkel sessions. Tests: **`userProfile_snorkelActivityOwnerInverse_cascadeDeletesOwnedSnorkels`**, **`appSwiftDataDualStoreMigrator_copiesSnorkelRowsIntoSplitContainer`**.
- **Bug fix — infinite launch loading:** Splash no longer waits on full Firebase + iCloud population (up to ~3×90s). **`restoreSession`** attaches with a **25s** CloudKit profile cap, dismisses overlay, then runs **`AccountRemoteDataPopulation`** in the background (`holdsLaunchOverlay: false`). **`ProductionAppRoot`** registers the CloudKit reconnect handler before session restore; stale **`pendingICloudDiveLogReconnect`** clears when private sync is already enabled; post-sign-in reconnect no longer re-shows the splash.
- **Bug fix — empty Logbook after CloudKit sign-in:** Background **`syncCloudKitDiveLogIntoSession`** polls private CloudKit import (up to ~90s), merges Apple-ID profile twins, and re-attaches the session to the profile that owns dives/snorkels; runs after deferred session restore and Sign in with Apple. **`AccountSessionProfileResolution.lookup`** prefers the oldest profile when all twins are still empty; CloudKit identity observer re-attaches when import lands. Test: **`accountSessionProfileResolution_totalOwnedActivityCount_sumsAcrossProfiles`**.
- **CloudKit import speed:** Firebase population and iCloud dive-log sync now run **in parallel** after launch/sign-in; **`GoDiveCloudKitDiveLogSyncKickstart`** nudges mirroring on store open; **`GoDiveCloudKitPrivateImportNotification`** wakes the sync loop on import events (150ms fallback poll vs 500ms blind wait); identity observer retries are tighter; **`GoDiveCloudKitForegroundImportWindow`** keeps the process warm ~8s while the log is still empty. Tests: **`goDiveCloudKitPrivateImportNotification_*`**, **`accountSessionProfileResolution_cloudKitImportPollInterval_isFasterThanIdentityObserver`**.
- **Snorkel CloudKit upload parity:** **`SnorkelActivity`** rows mirror to private CloudKit with dives (**`AppSwiftDataStorePartition.userModelTypes`**). **`SnorkelActivityOwnership.claimUnownedSnorkels`** + **`DiveUnownedClaimGate`** now include snorkel orphans (pre-sign-in FIT imports were never claimed). **`SnorkelActivity.swimTrackData`** (**`SnorkelSwimTrackCodec`**, LZFSE) syncs GPS/HR tracks like **`DiveActivity.profileTrackData`**; FIT import + **`SnorkelSwimTrackBackfill`** encode missing blobs for export. **`AccountRemoteDataPopulation`** schedules CloudKit reconcile when owned snorkels are zero too. Tests: **`snorkelActivityOwnership_claimUnowned_assignsOrphanSnorkels`**, **`snorkelSwimTrackCodec_roundTripsGPSAndHeartRateSamples`**, **`snorkelProfilePointStore_materializesLocalRowsFromSwimTrackBlob`**.
- **Bug fix — dive overview sheet scroll after Weather:** Weather made the map panel taller than the viewport; **`OverviewPanelScrollArea`** wrongly injected a scroll-restoration top inset on the first live scroll (>4 pt) instead of only when returning from nested navigation — scroll then felt stuck. Fix: **`DiveActivityOverviewScrollRestoration.shouldApplyScrollRestorationInset`** (fallback-only) + bound **`ScrollView`** height to the panel body. Test: **`diveActivityOverviewScrollRestoration_insetOnlyWhenReturningFromNestedNavigation`**.
- **Bug fix — map / tank overview scroll glitch:** Live scroll offset was also written to **`DiveActivityOverviewUIStateStore`** and fed back as **`scrollRestorationFallbackY`**, re-injecting the top inset on every scroll tick. Fix: fallback only when the live binding reads ~0; inset requires **`persisted < 4`**; remount scroll on tab change via **`panelScrollContentIdentity`**; reset offset when switching map / tank / media. Tests: **`diveActivityOverviewScrollRestoration_insetOnlyWhenReturningFromNestedNavigation`** (live-scroll cases).
- **Import-time weather persistence:** FIT / UDDF dive and FIT snorkel import now call **`ActivityWeatherImportCapture`** after site link + timezone resolution — **`ActivityWeatherKitService.fetchPersisted`** freezes canonical metric fields in **`activityWeatherSnapshotData`** on **`DiveActivity`** / **`SnorkelActivity`** (soft-fail when WeatherKit unavailable). Map overview reads the stored snapshot via **`ActivityWeatherSnapshotStorage`** and skips live WeatherKit when present; older dives without a blob still live-fetch. Tests: **`activityWeatherPersistedSnapshot_codecRoundTrips`**, **`activityWeatherSnapshotStorage_rebuildsDisplayForUnits`**.

## 120 - Tank chart, Buddy Feed, snorkel heroes **(pushed)**

- **Tank landscape depth chart:** dive **Tank** tab in landscape uses edge-to-edge profile chart framing (full width, band from tab chrome to bottom safe area) with **`.edgeToEdge`** chrome (no axis gutters). **`DiveTankOverviewHeroPresentation.minimizedLandscapeProfileChartFrame`**, **`DiveTankOverviewHeroView`**. Test: **`diveTankOverviewHeroPresentation_landscapeMinimizedProfileChart_isEdgeToEdge`**.
- **Tank landscape chart side buffer:** **7%** non-scrubbable margins on left/right; static underfill extends flat into gutters but depth + gas **lines** stay in the data band only (**`landscapeHorizontalEdgeBufferFraction`**, **`depthProfileUnderfillBoundaryPath`**). Tests: **`diveDepthProfileOverlayChartLayout_chartX_landscapeBuffer_insetsDataBand`**, **`…_elapsedSeconds_rejectsLandscapeBufferMargins`**, **`…_tracedProfilePath_extendsFlatIntoBufferForFillOnly`**.
- **Depth chart fill flip:** shimmering water + miniature bubbles now fill **above** the depth line (surface band); static deep-blue gradient sits **below** the curve. Water-fill gradient is **lighter toward the surface (top)** and **darker toward the depth line / plot floor** (**`surfaceGradientTop`** → **`surfaceGradientBottom`**). **`DiveDepthProfileChartWaterFillView`**, **`DiveDepthProfileChartStaticUnderfillView`**.
- **Depth chart gas-line scale:** PSI/bar line uses the depth axis band — max pressure at min recorded depth, min pressure at max recorded depth — so the lowest gas reading sits at the same **y** as deepest depth with the same **20%** headroom below (not pinned to the plot floor). **`DiveDepthProfileOverlayChartLayout.depthDataVerticalBand`**, **`pressurePoint`**. Test: **`diveDepthProfileOverlayChartLayout_pressurePoint_alignsWithDepthBandAndBuffer`**.
- **Depth chart gas-line smoothing:** PSI/bar polyline is averaged into **5 s** buckets then lightly neighbor-smoothed for display; scrub still resolves every raw sample. **`downsampledPressureSamplesForLine`**. Tests: **`diveDepthProfileChartPresentation_downsampledPressureSamplesForLine_*`**.
- **Depth chart scrub callout:** touch-and-hold scrub keeps the vertical line + depth/gas dots on the finger; time / depth / pressure label pins centered below the dive icon tab bar (**`iconTabBarChromeRowHeight`**, **`gapBelowTabMenu`**) in **`view_single_activity`** at **large** / landscape; portrait **minimized** pins the label in the chart top fade band (**`labelTopPaddingPinnedAtMinimizedPortraitChartFade`**). Tests: **`diveDepthProfileScrubCalloutPresentation_labelTopPadding_sitsBelowTabMenu`**, **`…_pinsAtMinimizedChartFade`**.
- **Tank chart full-bleed top:** portrait **large** and **landscape** depth charts start at **`profileChartBandTop`** (physical top of the hero; tab chrome overlays).
- **Tank chart large-detent seam bleed:** portrait **large** depth chart nudges **8 pt** below the sheet seam (**`largeDetentSheetSeamCornerBleed`**) so the plot tucks under the blue panel’s rounded top corners; **minimized** stays flush to the seam.
- **Tank minimized chart seam:** portrait tank depth chart matches **large** — edge-to-edge (no axes), full width, same fixed size, **`largeDetentSheetSeamCornerBleed`** at the sheet seam, and a transparent top fade (**`minimizedPortraitChartTopFadeFraction`**). **`DiveDepthProfileOverlayChart.topEdgeFadeFraction`**. Tests: **`diveTankOverviewHeroPresentation_minimizedProfileChartFrame_matchesLargeSizeAndSeamAligned`**, **`…_keepsSizeWhileSeamMoves`**.
- **Depth chart top headroom:** **15%** empty band above **0** depth (standard chrome); **large** / edge-to-edge tank hero adds another **15%** (**30%** total) via **`depthAxisLargeDetentAdditionalTopBufferFraction`**. Tests: **`diveDepthProfileChartPresentation_depthPlotY_reservesFifteenPercentTopBuffer`**, **`…_largeDetentTopBuffer_isThirtyPercentTotal`**.
- **Logbook — remove My Activities summary line:** dropped centered **`x Dives | x Bottom Time`** (and loading spinner) under the scope toggle on **Activity Log**; Home lifetime stats still show dive totals.
- **Tank minimized PSI entrance:** shimmering water above the depth curve is **fully visible** before the **5 s** draw; depth + gas lines and the darker under-curve fill wipe in together over that backdrop (**`DiveDepthProfileOverlayChart`**, **`DiveDepthProfileChartStaticUnderfillView`**).
- **Tank minimized rotate hint:** portrait **minimized** **`DiveTankRotatePhoneHintView`** moves from the chart/sheet gap to the embedded sheet’s upper trailing corner (**`minimizedPortraitRotateHintTopInset`**). Test: **`diveTankOverviewHeroPresentation_minimizedRotateHintTopInset_sitsBelowGrabber`**.
- **Snorkel overview hero tabs:** **Map** tab hero shows GPS **swim track** (**`SnorkelSwimTrackMapView`**); **Heart rate** tab hero shows **`SnorkelHeartRateProfileChart`** via **`SnorkelHeartRateOverviewHeroView`** (stats + calories stay in the panel). Test: **`snorkelHeartRateOverviewHeroPresentation_chartContentInsets_accountsForObstructions`**.
- **Home featured media crop:** carousel slides use band aspect-fill aligned to the blue-sheet seam (**`featuredMediaBandRect`** / **`featuredMediaBottomYFromTop`** with viewport height + **`featuredMediaBleedBelowSeam`**). Test: **`homeMediaCarouselPresentation_featuredMediaBandRect_endsAtSheetSeam`**.
- **Home buddy fan gap:** **`taggedBuddyHorizontalOffsetX`** no longer skips the slot beside the buddy icon — first avatar sits flush next to the icon when expanded.
- **Home buddy strip scroll:** expanded buddy avatars scroll horizontally when they overflow the chrome width; carousel pager **`scrollDisabled`** while buddy list (or fish overlay) is open so swipes do not change media.
- **Buddy Feed activity tiles:** **Buddy Feed** shows friend-shared dives and snorkels as card tiles with hero headers — depth chart for dives, GPS swim track map for snorkels — plus site name, region/country, key stats, and friend attribution. Firebase **`sharedDives`** projections (schema v2) now include **`activityKind`**, **`region`**, **`country`**, snorkel **`swimDistanceMeters`**, and **`swimTrackBase64`**; republish mirrors owned snorkels when **Share dives with friends** is on. **`LogbookBuddyFeedTileView`**, **`FriendSharedDiveDetailView`** hero. Tests: **`sharedSnorkelProjection_includesKindDistanceAndTracks`**, **`buddyFeed_tileStatsLine_formatsDiveAndSnorkel`**, **`friendShareAffectedDiveIDs_includesSnorkelActivities`**.
- **Buddy Feed depth profile fix:** friend share upsert now encodes **`profileTrackData`** from local **`DiveProfilePoint`** rows before Firestore upload; one-time **`GoDiveFriendShareProfileTrackRepublish`** schedules a full republish so existing shares gain **`profileTrackBase64`**. Buddy tiles/detail use tank-tab **`DiveDepthProfileOverlayChart`** via **`FriendSharedDepthProfileChartView`** (depth + gas line when present). Tests: **`sharedDiveProjection_decodedDepthChartSeries_includesPressureBaseline`**, **`friendShareProjection_encodesProfileTrackFromLocalPoints`**.
- **Buddy Feed depth chart reliability:** Firestore **`Timestamp`** → **`Date`** parsing for **`startTime`** (chart decode was failing on device reads); **`DiveProfilePointStore.profileTrackDataForSharing`** fetches local rows when the transient cache is empty; **`rowsEqual`** compares full buddy rows (including **`profileTrackBase64`**) so tiles refresh after republish; republish one-shot bumped to **v2**. Tests: **`sharedDiveProjection_parseFriendVisibleDive_readsFirestoreTimestamp`**, **`buddyFeed_rowsEqual_comparesProfileTrackPayload`**, **`friendShareProjection_profileTrackDataForSharing_encodesFromFetchedPoints`**.
- **Buddy Feed chronological sort:** merged friend activities sort newest-first by **`startTime`** (full date + time), then **`diveNumber`**, with **`updatedAt`** fallback; per-friend Firestore fetches re-sort client-side. Tests: **`buddyFeed_sortsSameDayActivitiesByStartTime`**, **`buddyFeed_sortFriendVisibleDives_fallsBackToUpdatedAt`**.
- **Snorkel friend share:** snorkel sessions upload to **`sharedDives`** with **`activityKind: snorkel`**, swim distance, and **`swimTrackBase64`** (from local GPS/HR points via **`SnorkelProfilePointStore.swimTrackDataForSharing`**); FIT snorkel import schedules friend-share republish; save observer covers snorkel media/buddy/sighting edits; one-shot republish bumped to **v3**. Settings copy clarifies dives **and snorkels** share under the same toggle. Tests: **`friendShareProjection_encodesSwimTrackFromLocalPoints`**, **`friendShareAffectedDiveIDs_includesSnorkelMediaPhoto`**.
- **Snorkel map hero zoom:** **`SnorkelSwimTrackMapPresentation.fittingMapRect`** fits the GPS polyline tightly (10% padding, 45 m floor); snorkel **Map** tab uses reduced hero-band insets; Buddy Feed / friend-shared snorkel heroes use **`.compact`** camera fitting. Tests: **`snorkelSwimTrackMapPresentation_fittingMapRect_isTighterThanRegionPadding`**, **`snorkelSwimTrackMapPresentation_compactCameraPadding_isMinimal`**.

## 121 - Buddy Feed detail, tank hero polish **(pushed)**

- **Buddy Feed shared detail shell:** tapping a Buddy Feed tile opens **`FriendSharedDiveDetailView`** with the same map / tank (dives) or map / heart rate / media (snorkels) hero + embedded overview panel as owned activity detail — read-only panels omit notes, tags, buddies, marine life, equipment, and media when not shared. **`FriendSharedActivityDetailPresentation`**, **`FriendSharedActivityDetailPanels`**. Tests: **`friendSharedDetail_diveNumberChip_nilForSnorkel`**, **`friendSharedDetail_mapCoordinate_rejectsInvalid`**, **`friendSharedDetail_snorkelDerivedSnapshot_decodesSwimTrack`**.
- **Buddy Feed tile media hero pager:** when shared media exists, tile heroes start on the owner's **featured** preview and support horizontal paging through other shared photos, then the depth chart (dives) or swim map (snorkels). Firestore projections now include **`featuredMediaPhotoId`**; upload order prefers featured first. **`LogbookBuddyFeedMediaHeroPage`**. Tests: **`buddyFeed_heroPages_featuredMediaFirstThenChart`**, **`buddyFeed_heroPages_chartOnlyWithoutMedia`**, **`buddyFeed_rowsEqual_comparesMediaPayload`**.
- **Buddy Feed tile hero height:** **`LogbookBuddyFeedTileLayout.heroHeight`** doubled (**128 → 256** pt) for media / depth-chart / swim-map pager headers.
- **Buddy Feed hero swipe vs navigation:** when a tile has shared media, the hero pager sits **outside** **`NavigationLink`** so horizontal swipes page media → depth chart; tap the stats body to open detail. Pager uses horizontal **`ScrollView`** paging. **`LogbookBuddyFeedNavigableTile`**.
- **Buddy Feed tile hero pages:** at most **one** featured/shared media page plus depth chart (dives) or swim map (snorkels); media-only, chart-only, or placeholder when data is missing. Tests: **`buddyFeed_heroPages_mediaOnlyWithoutDepthData`**, **`buddyFeed_heroPages_placeholderWhenNoMediaOrDepthData`**.
- **Buddy Feed pagination:** first **20** activities show when the feed loads; scrolling to the bottom loads the next **20** from the merged cache (**`LogbookBuddyFeedPresentation.pageSize`**). Test: **`buddyFeed_pagination_loadsTwentyAtATime`**.

- **Phase 1 — buddy shared media schema v3:** Firestore **`mediaItems[]`** (kind, thumbnailURL, contentURL, dimensions); **`GoDiveSharedMediaLimits`** (20 photos, 10 videos, 30 s video); tiered Storage paths (`thumb.jpg` / `photo.jpg` / `video.mp4`) in **`GoDiveSharedMediaStorage`**; nested + legacy **`storage.rules`**; v2 **`mediaPreviews`** read fallback. Design: **`cursor/friend_shared_media_hq_design.md`**.
- **Phase 2 — buddy shared media export + upload:** **`GoDiveSharedMediaSelection`** (20 photo / 10 video caps, featured-first order); **`GoDiveSharedMediaExport`** (256 px thumb, up to 4096 px JPEG, 1080p / 30 s MP4); **`GoDiveSharedMediaUpload`** wires tiered Storage uploads for dives + snorkels via **`GoDiveSharedDiveProjectionSync`**. Tests: **`goDiveSharedMediaSelection_*`**, **`sharedSnorkelProjection_writesMediaItemsWhenMediaOptIn`**.
- **Phase 3 — buddy shared media queue + cleanup:** Two-phase publish (thumbs → Firestore → background content patch); **`GoDiveSharedMediaPublishState`** fingerprint dedup; **`GoDiveSharedMediaUploadQueue`** (2 concurrent content uploads, resumable video via **`putFileAsync`**); per-item Storage delete + **`FieldValue.delete()`** on media/notes opt-out; **Share media on Wi‑Fi only** setting; Wi‑Fi reconnect triggers friend-share republish. Tests: **`goDiveSharedMediaPublishState_*`**, **`sharedDiveProjection_applyOptOutFieldDeletes_*`**, **`appNetworkConnectivityPresentation_friendShareContentUpload_*`**.
- **Dive overview motion snappiness:** grabber drag throttles live height publish (**`liveHeightFractionPublishEpsilon`**); MapKit / Google Maps skip mid-drag camera + padding updates; map panel content no longer springs on every live fraction tick; dive/friend hero drops implicit detent animation (panel springs locally); tank minimized 5s entrance waits for detent spring to settle; depth scrub uses binary nearest-sample lookup, pending-finger storage (no body invalidation during hold), **`Equatable`** static plot layer so O(n) paths are not rebuilt every scrub frame, and pauses water-fill TimelineViews while scrubbing. Tests: **`diveActivityOverviewPanelMetrics_shouldPublishLiveHeightFraction_throttlesSmallDeltas`**, **`diveDepthProfileSeries_indexNearestElapsed`** mid-span cases.
- **Buddy shared dive tank hero:** friend dive detail **Tank** tab reuses **`DiveTankOverviewHeroView`** with the same fixed large chart size + seam translate, top fade, cylinder / gas summary at **minimized**, and scrub label placement as owned dive detail. **Landscape** hides the overview panel and shows the same edge-to-edge depth chart (7% side buffers, bubbles, zoom/pan after settle) as owned **Tank → landscape**; orientation unlock treats **`.buddySharedDive`** like other dive activity routes. Tests: **`friendSharedDetail_tankHeroPresentation_matchesOwnedDiveRules`** (portrait + landscape frames), **`appPortraitOrientationLockPolicy_listScreensStayPortrait`** buddy path.
- **Tank depth chart bubbles:** rising bubbles only on portrait **minimized** and **landscape**; portrait **large** keeps the water fill without bubbles (**`showsAnimatedDepthChartBubbles`**). Test: **`diveTankOverviewHeroPresentation_showsAnimatedDepthChartBubbles_byDetent`**.
- **Tank large→minimized drag choreography:** while pulling the sheet down, depth + gas lines and the dark underfill fade out and a water backdrop fills the gap above the translating chart (water to the top of the screen). On snap to **minimized**, no panel-spring delay — top-half water fades + cylinder/PSI chrome fade in (**0.35 s**), then lines/underfill re-wipe and PSI tally (**2.4 s**, easeOut). Expanding **minimized→large** keeps the chart translating with the sheet and fades PSI/tank chrome out. **`FriendSharedDiveDetailView`** uses the same entrance state / detent handler as owned dive. Tests: **`diveTankOverviewHeroPresentation_collapseProgress_mapsLiveSheetHeight`**, **`…_profileStrokeOpacity_fadesWhileCollapsingFromLarge`**, **`…_minimizedChromeOpacity_fadesOnExpand`**, **`…_collapseWaterBackdropOpacity_clearsWithTopFade`**, **`friendSharedDetail_tankHeroPresentation_matchesOwnedDiveRules`** (**`tankMinimizedEntranceMatchesOwnedDive`**).
- **Tank minimized chart taller + large top fade:** portrait minimized depth chart grows to **1.6×** large resting height (clamped to the sheet seam) as the sheet collapses, giving more water space above; soft top transparency fade (**`portraitChartTopFadeFraction`**) is on from **large** so it remains while dragging. Owned + buddy via **`DiveTankOverviewHeroView`**. Tests: **`diveTankOverviewHeroPresentation_portraitChartHeightScale_growsWithCollapse`**, updated minimized frame tests.
- **Tank pull-down stroke fade:** blue depth + yellow gas lines and the dark under-curve fill fade out by **~22%** of the large→minimized drag; water/chrome fade-in **0.35 s**, line/underfill wipe + PSI tally **2.4 s** with **easeOut**. Test: **`diveTankOverviewHeroPresentation_profileStrokeOpacity_fadesWhileCollapsingFromLarge`**, **`…_minimizedEntranceAnimation_isSnappy`**.
- **Preflight:** **`LogbookBuddyFeedPresentation.HeroPage`** explicit **`nonisolated`** **`==`** (Swift 6); tank minimized frame tests aligned with **1.6×** chart scale.

## 122 - Buddy shared media Phase 4 (cache + progressive UI)

**Summary:** Friend-visible media loads progressively — disk-cached thumbnails in Buddy Feed, thumb→full crossfade and streaming video in detail, Firebase URL policy on all fetches, and **Download friend media on Wi‑Fi only**.

- **`GoDiveSharedMediaCache`:** LRU disk cache under **`Caches/GoDiveSharedMedia/`** (`thumbs/` ~50 MB, `content/` ~500 MB); all URLs gated through **`GoDiveRemoteURLPolicy`**.
- **`FriendSharedMediaPresentation`:** v3 **`mediaItems`** + v2 fallback; featured-first ordering; buddy-feed / detail prefetch URL helpers; content-download policy (Wi‑Fi-only + Low Data Mode).
- **`FriendSharedMediaImageView`:** thumb-first stills with optional progressive crossfade to **`contentURL`**.
- **`FriendSharedRemoteVideoPlayerView`:** poster thumb + muted looping **`AVPlayer`** stream on **`contentURL`**.
- **Buddy Feed:** thumb-only heroes with video badge; prefetch next **3** thumbnails on scroll.
- **Friend detail:** progressive photo heroes, video playback, grid thumbnails, adjacent content prefetch on pager swipe.
- **Settings:** **Download friend media on Wi‑Fi only** (default off; thumbnails still load on cellular).
- Tests: **`friendSharedMedia_*`**, **`goDiveSharedMediaCache_storesAndReadsThumb`**; updated **`buddyFeed_heroPages_*`** for **`DisplayItem`** heroes.

## 123 - Buddy shared media Phase 5 (fullscreen zoom + privacy)

**Summary:** Friend activity photos open in a fullscreen browser with pinch-to-zoom on full-quality stills; privacy guide updated for HQ shared media and on-device friend cache.

- **`FriendSharedMediaFullscreenView`:** horizontal pager, page counter, close control; photos use **`FriendSharedMediaZoomableImageView`**; videos stream via **`FriendSharedRemoteVideoPlayerView`**.
- **`FriendSharedMediaFullscreenPresentation`:** zoom clamp + chrome helpers.
- **Friend detail:** tap media hero or grid thumbnail → **`fullScreenCover`**; grid also switches to the media tab first.
- **`docs/friends.md`**, **`docs/privacy-and-data.md`:** fullscreen zoom, shared media tiers, Wi‑Fi download setting, device-side friend media cache.
- Tests: **`friendSharedMediaFullscreen_*`**, **`goDiveSharedMediaExport_sharePhotoJPEG_respectsSizeCap`**.

## 124 - Buddy shared media Phase 6/7 (export hygiene + test audit)

**Summary:** Closed remaining design-matrix gaps — GPS-stripped share JPEGs, video duration cap helper, cap-trim logging, cache LRU eviction test, and download Wi‑Fi policy test. Storage rules deployed to **`godive-1cff8`**.

- **`GoDiveSharedMediaExport`:** **`normalizedSharePhotoImage`** re-encodes through a bitmap context (strips location EXIF); **`cappedVideoExportDurationSeconds`**; **`jpegContainsGPSMetadata`** test helper.
- **`GoDiveSharedMediaSelection`:** **`capTrimSummary`**, **`trimNoticeMessage`**, **`preservesGalleryOrder`**; upload logs when caps trim items.
- **`GoDiveSharedMediaCache`:** **`testingTierMaxBytes`** for LRU eviction tests.
- Tests: design-matrix coverage (**12 videos → 10**, **60 s → 30 s**, gallery order, LRU eviction, download Wi‑Fi gate, GPS-free JPEG).
- **Ops:** **`catalog-cdn/storage.rules`** deployed — `npx firebase-tools@latest deploy --only storage --project godive-1cff8` (rules compiled + released)

## 125 - Buddy media tab matches owned activity shell

**Summary:** Friend shared activity **Media** tab now mirrors owned dive/snorkel media UX — horizontal hero pager, minimized carousel strip, large-detent read-only species/buddy detail — without import, tagging, or featured editing.

- **`FriendSharedActivityMediaPanelContent`:** carousel at **minimized**; dive identity + read-only tag summaries; **`DiveActivityMediaLargeDetentOverviewContent`** at **large** (activity-level sightings/buddies).
- **`FriendSharedActivityMediaCarouselView`:** featured star display-only; thumbnail selection syncs hero (no auto-fullscreen).
- **`FriendSharedActivityMediaHeroView`:** paging **`ScrollView`** hero, capture-date Liquid Glass chip above minimized sheet (**`captureOverlayBottomInset`**, full-bleed hero margin), landscape playback chrome, hero tap → pinch-zoom fullscreen.
- **`FriendSharedMediaPresentation.resolvedFeaturedMediaID`**, **`FriendSharedActivityDetailPresentation.displayMarineLife` / `displayBuddies`**.
- Tests: **`friendSharedMediaPanelPresentation_*`**, **`friendSharedActivityDetailPresentation_mapsReadOnlyMediaTagModels`**.

## 126 - Per-activity buddy sharing settings **(pushed)**

**Summary:** Owned dive and snorkel detail screens get an edit control for per-activity buddy sharing — activity/media/notes toggles, checkbox media grid, and a Firebase-backed status footer.

- **`ActivityFriendShareConfiguration`**, **`ActivityFriendShareNotesMode`:** per-activity share resolution — unconfigured activities follow global Settings; configured activities use stored overrides only (global media/notes changes do not alter them).
- **`DiveActivity` / `SnorkelActivity`:** `friendShare*` fields; sync/upload honor explicit media selection when configured.
- **`ActivityFriendShareSettingsView`**, **`ActivityFriendShareMediaSelectionView`:** settings sheet from **`ViewSingleActivity`** / **`ViewSingleSnorkelActivity`** (horizontal **⋯** toolbar).
- **`ActivityFriendShareStatusPresentation`:** Firestore projection check + upload queue → **Sharing Off** / **Activity Upload in Progress** / **Activity Shared**.
- **Sharing off cleanup:** per-activity or global disable deletes **`sharedDives/{activityId}`**, cancels in-flight uploads, wipes Storage media, and clears local publish cache for that activity.
- **Buddy Feed video:** featured shared clip auto-plays once (muted) when scrolled into view; friend activity detail prefetches all shared video content on open.
- **Friend-shared video playback:** cache-first **`AVPlayer`** (local disk when prefetched), parallel content prefetch (up to 3 concurrent), background prefetch while streaming remote; Buddy Feed uses faster start buffering.
- **Friend-shared photo full-res:** Buddy Feed hero + detail use progressive thumb→content; reload when **`contentURL`** arrives; detail re-fetches Firestore projection on open; prefetch all photo + video content tiers.
- **Dive Media video crash (detent drag):** unified hero layout keeps one view tree across minimized↔large sheet transitions so **`AVPlayer`** is not dismantled mid-drag; preview→full upgrade works when carousel disables session reuse.
- Tests: **`activityFriendShareConfiguration_*`**, **`activityFriendShareStatusPresentation_*`**, **`friendSharedMedia_allVideoContentPrefetchURLs_*`**, **`goDiveSharedMediaCache_resolvedPlaybackURL_*`**, **`friendSharedMedia_resolvedVideoPlaybackURL_*`**, **`diveActivityMediaHeroPresentation_interpolatedSize_fillsViewportAtFullProgress`**.
- **Warning fix (device build):** `DiveActivityOverviewDetent.defaultSelection` marked `nonisolated` (matching the enum's other members) so the `DiveActivityOverviewLiveSheetState.init` default argument no longer trips a main actor-isolation warning.
- **Tank chart real-time drag (owned + friend dive):** new **`DiveActivityOverviewLiveSheetState`** (`@Observable`) carries the **unthrottled** grabber height so only the tank hero re-evaluates per drag frame — the page keeps its epsilon-throttled `@State` for map/media. Chart now mounts at its **resting** frame and follows the seam via **`scaleEffect` + `position`** (render-server transform; `Equatable` plot layer untouched mid-drag), interpolating **large ↔ minimized** geometry through **`collapseProgress`** / **`interpolatedPortraitProfileChartFrame`**. Collapse stroke fade hoisted **outside** the equatable plot layer (split **`DiveDepthProfileWaterFillPlotLayer`** + strokes layer) so per-frame opacity no longer rebuilds O(n) paths. On release the panel sets the resting fraction **inside** the shared `.diveOverviewPanelDetent` spring (hero replaces its `easeInOut` detent animation with the same spring) so chart and sheet settle together. Drag release position is recorded as **`dragReleaseHeightFraction`** and consumed by the detent-change handlers — a long drag to **minimized** skips the 2.4 s line-wipe replay (**`shouldSkipMinimizedEntranceAfterDrag`**), while tap-to-expand/collapse always plays it. Tests: **`…_portraitChartDragTransform_identityAtRestTracksTarget`**, **`diveActivityOverviewLiveSheetState_defaultsToRestingDetentAndClearsRelease`**, **`…_shouldSkipMinimizedEntranceAfterDrag_nearMinimized`**, **`…_interpolatedPortraitProfileChartFrame_blendsLargeAndMinimized`**.
- **Shared media publish reliability (live-cloud audit fixes):** production Firestore/Storage showed photo `photo.jpg` content had **never** uploaded and photo-activity projections were stuck on schema v2 — thumbnails re-uploaded across days while the doc never updated. Root causes fixed:
  - **PhotoKit stall timeout for every delivery mode** (`DiveMediaReferenceLoader.fetchImageFromPhotoKit`): `.highQualityFormat` share/export requests previously had **no** deadline, so one stalled iCloud original froze the whole publish loop before the Firestore write. All modes now resolve within `DiveMediaStillLoad.requestTimeoutSeconds` (nil → item soft-skipped).
  - **True two-phase publish** (`GoDiveSharedMediaUpload`): phase one uploads **thumbnails only** (stored 256 px preview bytes; no 4096 px export, no video transcode) and checkpoints `GoDiveSharedMediaPublishState` after **every** item, so interrupted publishes resume without re-uploading. Content jobs now carry just `photosLocalIdentifier`; the background queue exports the 4096 px JPEG / 1080p MP4 lazily, uploads with 2-way concurrency, checkpoints each completed object, and patches Firestore `mediaItems` once at the end. Photo dimensions/bytes/fingerprint recorded at content time (`sha256Hex` of content bytes).
  - **Coordinator no longer cancels in-flight publishes** (`GoDiveFriendShareRefreshCoordinator`): debounce timer and drain are separate tasks — a SwiftData save mid-publish used to cancel the running flush (killing the Firestore upsert); now new work queues behind the active drain and flushes after it. Legacy v1/v2 `sharedDives` docs migrate to v3 automatically on the next full republish (buddies list open, settings toggle, invite redeem, connectivity restore).
- **Buddy sharing status checklist (activity settings footer):** single status label replaced with an **Upload in progress…** banner (spinner) plus a three-row checklist — **Activity / Media / Notes**, each **Shared** ✓, **Uploading**, or **Off**. `ActivityFriendShareStatusPresentation` gains `ShareStatusChecklist` + pure `shareStatusChecklist(...)` mapper (Firestore snapshot now carries `mediaItemCount` + `hasNotesField`) and `notesExpected(mode:privateNotes:publicNotes:)` (notes only count when text is non-empty, mirroring the projection mapping). `ActivityFriendShareConfiguration.DisplayStatus` removed. Footer polls every 3 s while anything is uploading.
- Tests: **`activityFriendShareStatusPresentation_checklist_*`** (sharing off → nil, all shared, content pending / queue busy, missing doc), **`activityFriendShareStatusPresentation_notesExpected_respectsModeAndText`**, **`goDiveSharedMediaPublishState_sha256Hex_changesWithContent`** (replaces removed combined `exportFingerprint` helper test).
- **Buddy share copy + defaults:** Settings toggles renamed to **Share activities with buddies**, **Share notes with buddies**, **Share media with buddies** (and **Download buddy media on Wi‑Fi only**); per-activity **Buddy Sharing** toggles use matching **with buddies** copy. New activities snapshot global defaults at creation (`friendShareBuddyDefaultsCaptured`); existing activities backfill on launch (`ActivityFriendShareDefaultsMigration`) so later global Settings changes apply only to new entries — configured per-activity overrides unchanged. Tests: **`activityFriendShareConfiguration_unconfiguredFollowsGlobalUntilCaptured`**, **`activityFriendShareConfiguration_seedBuddyShareDefaultsOnNewActivity`**.
...
- **Cloud Function `notifyBuddyActivityShared`** (`catalog-cdn/functions/index.js`): `onDocumentCreated("users/{uid}/sharedDives/{activityId}")`. Server-only state doc **`buddyActivityPush/{posterUid}`** (default-deny rules; Admin SDK only) holds a permanent `notifiedActivityIds` dedupe list (FIFO-capped at 5000) plus a `pending` queue. The invocation that opens a 20 s window sleeps, transactionally drains the queue, then sends **one** multicast push to all active-`friendships` members (minus the poster), skipping receivers whose `users/{uid}/private/notificationPrefs.buddyActivitySharesEnabled == false`. Copy: *"{Name} logged a new dive/snorkel."* / *"{Name} shared N new activities."*; `apns-collapse-id` per poster so bursts replace instead of stacking. Payload `data`: `type=buddy_activity_shared`, `friendUID`, `activityID` (latest by dive `startTime`), `activityCount`.
- **`GoDiveBuddyActivityPushPresentation`** (new): payload keys + `target(fromUserInfo:)` parsing, copy helpers mirroring the server, `latestActivityID(from:)` parity helper, and `GoDiveBuddyActivityPushNavigationStore` (pending tap target across cold launch, same pattern as post-redeem friend profile).
- **Tap routing:** `GoDiveFirebaseCloudMessaging.handleNotificationResponse` recognizes the new type → stores target + posts `openBuddySharedActivityNotification`. `ContentView` consumes on receive / `onAppear` / `showsMainAppShell` → Logbook tab + pending `LogbookRoute.buddySharedDive`. `LogbookView.consumePendingLogbookRouteIfNeeded` special-cases that route: switches scope to **Buddy Feed**, pops to root, refreshes the feed, and pushes the activity only once its row is loaded (`LogbookBuddyFeedPresentation.containsRow`) — missing/unshared targets stay on the feed.
- **Settings — Buddy activity notifications** (default **on**, `goDiveNotifyBuddyActivityShares`): new toggle under the buddy-media rows. `GoDiveBuddyActivityPushPreferenceSync` mirrors it to Firestore `users/{uid}/private/notificationPrefs` (owner-only rules already cover the `private` subcollection — no rules change) and pulls the server value on Settings open; enabling also runs push permission/token registration.
- Tests: **`buddyActivityPush_target_parsesValidPayload`**, **`buddyActivityPush_target_rejectsWrongTypeOrMissingKeys`**, **`buddyActivityPush_notificationCopy_singleAndBatched`**, **`buddyActivityPush_latestActivityID_picksLatestStartTime`**, **`buddyActivityPush_buddyFeedContainsRow_matchesFriendAndDocument`**, **`buddyActivityPush_navigationStore_consumeClearsPending`**, **`buddyActivityPush_notifyPreferenceDefaultsOn`**, **`homeNotifications_items_mergeAndSortNewestFirst`**, **`homeNotifications_items_skipFriendWithoutSinceDate`**, **`homeNotifications_activityDate_prefersUpdatedAtOverStartTime`**, **`homeNotifications_hasUnread_gatesOnLastSeen`**, **`homeNotifications_lastSeenStore_roundTrip`**.
- **Home Notifications page + bell:** new **bell** in the upper-left of the Home header (Liquid Glass circle, red unread dot) → pushes **`HomeNotificationsView`** (**`HomeRoute.notifications`**; `AppPage` collapsible **Notifications** title like Settings). Rows derive from Firestore social data — friendship `since` dates (*"{Name} is now your dive buddy"*) and friend-shared activities (*"{Name} logged a new dive/snorkel"* + site name), newest first, capped at 100 (**`HomeNotificationsPresentation.items`**) — so history is complete even when a push never reached this device. Rows push **`HomeRoute.friendProfile`** / **`HomeRoute.buddySharedActivity`** on the Home stack (row payload travels in the route — `FriendVisibleDive`, media/tag snapshots, and `LogbookBuddyFeedPresentation.Row` gained `Hashable`). Unread = newest item newer than per-profile last-seen (**`HomeNotificationsLastSeenStore`**, UserDefaults); opening the list marks seen. Badge fetch is one deferred (1.5 s) buddy-feed snapshot per profile session so Home's first frame stays clean. **`AppHeader`** gained an optional **`leadingContent`** slot (placed after `trailingContent` so existing trailing-closure call sites are unchanged); **`BlueSheetHomeTopChrome`** threads it through.
- Docs: `cursor/buddy_activity_push_notifications.md` (deploy + design), `docs/friends.md`, `docs/settings.md`, `docs/privacy-and-data.md` (token/pref storage + notification content), `docs/home.md` (bell + Notifications list).
- **Tagged-you notifications:** when a friend shares an activity that tags your Firebase account in **`taggedBuddies`**, Home **Notifications** adds a second row — *"{Name} tagged you in a new dive/snorkel"* (sorted above the generic *logged a new* row for the same activity). **`HomeNotificationsPresentation.items`** takes **`currentFirebaseUID`** (cached via **`GoDiveFirestoreUserProfileMapping`**) and reuses **`GoDiveSharedDiveProjectionMapping.wasCurrentUserTagged`**. **`notifyBuddyActivityShared`** reads **`taggedBuddies`** on each queued activity and sends per-recipient copy — tagged friends get *tagged you* wording (batched count when several in the 20 s window tag them); everyone else keeps the generic buddy-activity copy. **`GoDiveBuddyActivityPushPresentation.taggedYouNotificationBody`** mirrors server strings. Tests: **`homeNotifications_items_addTaggedYouRowWhenCurrentUserTagged`**, **`buddyActivityPush_taggedYouNotificationCopy_singleAndBatched`**.
- **Deployed:** `notifyBuddyActivityShared` **and** `notifyFriendInviteAccepted` to `godive-1cff8` (us-central1, 2nd gen) — first functions deploy for the project (Cloud Functions / Eventarc / Cloud Build APIs enabled; the invite function had never actually been live). Artifact cleanup policy set (images >1 day). Follow-up: Node.js 20 runtime is deprecated (decommission 2026-10-30) — bump `engines.node` + `firebase-functions` before then. **Redeploy `notifyBuddyActivityShared`** after this batch to ship tagged-you push copy.
- **Profile Diver stats fix:** **`ProfileDetailContentPage.usesStaticPagerLayout`** now treats **Diver stats** as static (like **Details** / trip stats) so **`HomeLifetimeStatsSection`**'s **`GeometryReader`** gets the full pager height instead of collapsing to zero inside a **`ScrollView`**.
- **Blue sheet panel body inset:** **`BlueSheetDetailPagePinnedSummaryPresentation.panelContentTopPadding`** (**`md`**, 16 pt) below **`BlueSheetDetailPanelContentTopDivider`** on all **`BlueSheetDetailPage`** shells so pager / stats content no longer sits flush on the hairline.
- **Profile Diver stats — no Top buddies:** **`HomeLifetimeStatsSection.includesBuddyLeaderboard`** is **`false`** on Profile (**`ProfileDetailContentPagerPresentation.showsBuddyLeaderboardOnDiverStats`**); lifetime tiles / leaderboard pushes remain. Home still always shows **Top buddies**. Test: **`profileDetailContentPagerPresentation_detailsPageCopy`**.
- **Profile Diver stats crash harden:** Profile no-buddy path skips **`GeometryReader`** centering (intrinsic top-aligned stack) so **`TabView`** page proposals cannot inflate **`.infinity`** clear spacers; **`resolvedVerticalEdgeInsets`** rejects non-finite heights; leaderboard / aggregate / Profile tagged-media maps use **`godiveUniquingKeysWithValues`**. Test: **`homeLifetimeStatsTilesLayout_resolvedVerticalEdgeInsets_rejectsNonFiniteHeight`**.
- **Buddy share upload stall:** friend-share flush **`await`s** in-flight content-tier uploads per activity; retries failed content jobs once; repairs snorkel/dive Photos local IDs before thumb export; oversized stored preview JPEGs scale for thumb tier; Firestore upsert failures keep disk pending work; checklist stops showing **Uploading** when the activity doc exists with zero media rows and no queue/local pending. Tests: **`activityFriendShareStatusPresentation_checklist_mediaOffWhenDocumentHasNoMediaRows`**.
- **Buddy activity push reliability (signal trigger):** client creates one-shot **`users/{uid}/buddySharePushSignals/{activityId}`** only when the projection doc is **first** created (republish skips — no bulk push on rebuild). **`GoDiveBuddyActivityPushSignalSync`**; signal deleted with projection delete. **`notifyBuddyActivityShared`** now **`onDocumentCreated`** on that collection (not **`sharedDives`** merges). CF fixes: stale batch-window recovery, skip clearing **`pending`** when no FCM tokens / zero send successes, dedupe only after successful **`sendEach`**, delete signal docs after send. Firestore rules: owner **`create`/`delete`** on **`buddySharePushSignals`**. **Deploy** rules + **`notifyBuddyActivityShared`** to `godive-1cff8`. Test: **`sharedDiveProjection_taggedBuddiesFirestoreRows_mapsFirebaseUid`**.
- **Activity delete in Activity Settings:** **⋯** on dive/snorkel detail opens **Activity Settings** (buddy sharing + **Delete dive** / **Delete snorkel** at the bottom). Removes the activity from SwiftData / CloudKit (tags, buddies, marine life denorm, trip links for dives, profile points, orphan dive sites) and deletes Firebase friend projections. Logbook swipe-delete removed. **`SnorkelActivityDeletion`** stack mirrors dive cleanup. Tests: **`snorkelActivityDeletion_*`**, **`snorkelActivityDeletionMarineLifeCleanup_*`**, **`snorkelActivityRelationshipDetachment_*`**.
- **One push per activity:** media/notes republishes must not notify again. Client sets **`friendSharePushSignalRecorded`** after the first signal write and skips further signals even if the projection doc is recreated. Cloud Function atomically **claims** the pending queue before FCM send (stops concurrent drains), restores the queue on failure, and drops Eventarc **retry** (retries were double-sending after APNs recovered). Test: **`buddyActivityPushSignal_shouldRecordOnlyOnFirstProjectionCreate`**. **Redeploy** `notifyBuddyActivityShared`.
- **Buddy push unblock:** Firestore rules had denied **read** on **`buddySharePushSignals`**, but the client **`getDocument`/`runTransaction`** create-once path needs owner read — every signal write failed silently, so **`notifyBuddyActivityShared`** never ran after the trigger moved off **`sharedDives`**. Rules now allow owner **read**; client writes an explicit **`Timestamp`** for **`startTime`** and uses a transactional create-once. CF: log FCM error codes, prune invalid tokens, reclaim stale **`pending`** windows on retry, **`retry: true`** + throw on zero FCM successes. **Deploy** rules + function. Prior log line *`1/1 failures for poster network`* means a buddy device token was rejected by FCM — open GoDive on the buddy phone (notification permission on) so a fresh **`fcm_*`** token uploads.
- **Local-first publish checkpoint (Strava-style):** new activities (FIT/UDDF import + manual) start **local-only** — `seedBuddyShareDefaultsOnNewActivity` forces `friendShareActivityEnabled = false` and sets new model flag **`friendSharePublishCheckpointPending`** (dives + snorkels; existing records default `false`, launch backfill unchanged). The owner edits (marine life, notes, media), then a banner at the top of the detail overview panel — *"This dive/snorkel is local only"* — offers one-tap **Share** (**`ActivityFriendSharePublishCheckpoint.publish`** → per-activity share on with seeded media/notes defaults, `scheduleUpsert` → projection + buddy push signal) or **Keep local** (configured-off, no publish). Saving Activity Settings also resolves the checkpoint (`applyConfiguredSettings` clears pending). New **`ActivityPublishCheckpointBanner`** (highlight-tile chrome, glass-prominent Share, brief *Shared with buddies* confirmation). Post-import republish leaves drafts untouched (`shouldPublish` already skips share-off activities). Tests: **`activityFriendShareConfiguration_seedBuddyShareDefaultsOnNewActivity`** (updated), **`activityFriendSharePublishCheckpoint_seedNewSnorkelStaysLocalWithPendingFlag`**, **`activityFriendSharePublishCheckpoint_backfillDoesNotSetPendingFlag`**, **`activityFriendSharePublishCheckpoint_applyConfiguredSettingsResolvesCheckpoint`**, **`activityFriendSharePublishCheckpoint_showsBannerMatrix`**, **`activityFriendSharePublishCheckpoint_publishSelectedMediaIDs`**, **`activityPublishCheckpointBanner_promptCopyPerActivityKind`**.

**Summary:** Navigation double-open safeguard — laggy multi-taps no longer stack the same page on itself.

- **`NavigationStackPushCoalescing`** — `append` skips when `path.last` equals the route; `coalescedByRemovingConsecutiveDuplicates` + **`coalescesNavigationStackPathDuplicates`** catch **`NavigationLink(value:)`** double-pushes; `assignIfNil` / `assignUnlessDuplicate` for optional nested destinations.
- Wired through Home / Logbook / Explore / Field Guide / Search push helpers; Profile / buddy / tag / trip item destinations guarded.
- Tests: **`navigationStackPushCoalescing_*`**.

**Summary:** Activity delete returns to Logbook with a 1-second checkmark — no “no longer in your log” placeholder.

- **`ActivityDeleteSuccessPresentation`** + **`ActivityDeleteSuccessCheckmarkOverlay`** — after Activity Settings delete, posts success; **`ContentView`** selects Logbook and shows a centered checkmark for **1 s**.
- **`LogbookView`** clears its navigation path on success; other stacks strip routes for the deleted activity ID.
- **`ActivityMissingDestinationPopView`** auto-pops missing dive/snorkel destinations instead of the black placeholder page.
- Tests: **`activityDeleteSuccessPresentation_*`**.

**Summary:** Logbook — swipe left/right between **My Activities** and **Buddy Feed**.

- **`LogbookListSurface`** — page-style **`TabView`** pager (dots hidden) bound to **`LogbookFeedScope`**; **My Activities** left, **Buddy Feed** right (matches the scope toggle). Both pages stay mounted so interactive swipes stay continuous; only the visible page drives header collapse.
