...
- **Dive Media minimized timestamp:** capture date chip on the hero uses shared **`MediaCaptureTimestampChromeLabel`** (Liquid Glass capsule + clock icon), portrait and landscape **minimized** detent.
...
- **Design — friend shared HQ media:** Added **`cursor/friend_shared_media_hq_design.md`** — two-tier thumb + content sharing (full JPEG / 1080p MP4), Firestore schema v3, Storage layout, two-phase upload, disk cache, dive+snorkel parity. Locked caps: **20 photos**, **10 videos**, **30 s** video clip, cellular default with optional Wi‑Fi-only settings, public Storage tokens (signed URLs later).
